...
volume_api_mutations = [
    ...
    ('inertial liquid interaction constraint', 'gpuparticles/volume/shaders/liquid-force.glsl',
     'v+=u_waterForceVector*u_interactionScale*pow', 'v+=u_waterForceVector*pow',
     'vertical interaction constraint x', 'volume-test'),
...
if options.mutations:
    mutations = [
        ...
        ('velocity stretch direction', 'gpuparticles/shaders/style.glsl',
         'localVertex.x+=(vertex.x-0.5)*u_stretch*speed;', 'localVertex.x+=(vertex.x+0.5)*u_stretch*speed;',
         'velocity stretch must extend'),
        ('carry', 'gpuparticles/shaders/simulate.glsl',
         'p+=u_carry*smoothstep(0.5,8.0,length(velocity))*stepTime;', 'p+=u_carry*smoothstep(0.5,8.0,length(velocity))*stepTime*0.0;',
         'stateful: carry must move a moving particle'),
