...
**F** switches to optional **water volume**. This runs a separate, persistent water grid across the entire scene: water falls from the source, spreads over rocks, fills low areas, and rises as more arrives. It does not require particle self-collision. The mouse applies a soft radial push, and pointer velocity adds a directional flux impulse while dragging. Neither interaction becomes solid or deletes density. A strong sustained push can make a shallow depression, while the surrounding flow refills it.
...
[Accumulation preview](../../previews/waterfall-volume.png) · [Mouse push preview](../../previews/waterfall-volume-mouse.gif)
...
The mouse remains a solid circular collider in particle mode. In volume mode it becomes a soft push with two blue rings. Scroll changes its radius and **C** toggles it. Both paths update uniforms without rebuilding terrain or uploading particle records.
...
Water-volume tests read float state to check gravity transfer, total mass against admitted source and overflow, solid-circle damming, soft-push conservation, lateral spreading, persistence with inflow off, and dynamic displacement. They check actual scene geometry/rendering, particle suspension/resumption, controls, resource cleanup, and unsupported-device fallback. `--water-volume-only` runs these tests and captures both standalone volume scenes; capture mode warms this otherwise initially empty scene for 12 seconds.
