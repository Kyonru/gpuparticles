local prefix=(...):gsub('example_support%.comparison%.view$','')
...
function V:draw(model)
  ...
  g.clear(palette.ink)
  g.setFont(self.title);g.setColor(palette.beige)
  ...
  g.setFont(self.small);g.setColor(palette.teal);g.printf(model.view=='both' and 'WINDOW FPS' or 'ISOLATED FPS',width-212,61,180,'right')
  ...
  local function panel(name,x,span)
    ...
    local blue=name=='native' and palette.blue or palette.teal
    g.setColor(palette.deep);g.rectangle('fill',x,top,span,panelHeight,8,8)
    ...
    if model.mouseCollision and model.pointer.inside then
      ...
      g.setColor(palette.peach[1],palette.peach[2],palette.peach[3],name=='gpu' and 0.16 or 0.05)
      g.circle('fill',imageX+p.x*size/512,imageY+p.y*size/512,p.radius*size/512)
      g.setColor(palette.peach[1],palette.peach[2],palette.peach[3],name=='gpu' and 1 or 0.4);g.setLineWidth(1.5)
  ...
  g.setFont(self.label);g.setColor(palette.beige)
